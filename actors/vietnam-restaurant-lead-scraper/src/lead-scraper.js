...
export const runLeadScraper = async ({ input, config }) => {
    const { searchLocation, searchTypes, maxResults } = resolveSearchConfig(input, config);
    const multiSearch = config.multiSearch;
    const searchQueryTemplate = multiSearch?.searchQueryTemplate ?? '{type} {location}';
    ...
    if (multiSearch) {
        if (!searchLocation) throw new Error(`Input "${multiSearch.locationInputField}" is required.`);
        if (!searchTypes.length) {
            throw new Error(`Input "${multiSearch.typeInputField}" must contain at least one search type.`);
        }
        if (!Number.isInteger(maxResults) || maxResults <= 0) {
            throw new Error(`Input "${multiSearch.maxResultsInputField}" must be a positive integer.`);
        }
    } else {
        if (!searchTypes[0]) throw new Error('Input "keyword" is required.');
    ...
    try {
        ...
        const collectPlacesForSearchType = async (type) => {
            ...
            const searchUrl = `https://www.google.com/maps/search/${encodeURIComponent(
                getSearchQuery(type, searchLocation, searchQueryTemplate),
            )}`;
            const seen = new Map();
            let noNewCount = 0;
        ...
        const extractPlace = async (place) => {
            ...
            try {
                ...
                const data = await detailPage.evaluate(() => {
                    ...
                    const category =
                        cleanText(document.querySelector('button[jsaction*="category"]')?.textContent) ?? null;
                ...
                const emailData = extractEmails
                    ...
                    : { emails: [], email: null };
                const searchTypeOutputField = multiSearch?.typeOutputField ?? 'keyword';
                const searchLocationOutputField = multiSearch?.locationOutputField ?? 'location';

                await pushDedupedItem({
                    [searchTypeOutputField]: place.searchType,
                    [searchLocationOutputField]: searchLocation,
