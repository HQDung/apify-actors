...
const city = String(input.city ?? input.location ?? 'Da Nang').trim();
const hotelTypes = (
    Array.isArray(input.hotelTypes) && input.hotelTypes.length
        ? input.hotelTypes
        : [input.keyword ?? 'hotel']
)
    .map((hotelType) => String(hotelType).trim())
    .filter(Boolean);
const maxResultsPerType = Number(input.maxResultsPerType ?? input.maxResults ?? 20);
...
if (!city) throw new Error('Input "city" is required.');
if (!hotelTypes.length) throw new Error('Input "hotelTypes" must contain at least one hotel type.');
if (!Number.isInteger(maxResultsPerType) || maxResultsPerType <= 0) {
    throw new Error('Input "maxResultsPerType" must be a positive integer.');
}
...
const collectPlacesForHotelType = async (hotelType) => {
    const { context: searchContext, page } = await createPage();
    const searchQuery = `${hotelType} ${city} Vietnam`;
    ...
    try {
        ...
        while (seen.size < maxResultsPerType && noNewCount < 5) {
            ...
            for (const place of places) {
                if (!seen.has(place.url) && seen.size < maxResultsPerType) {
                    seen.set(place.url, { ...place, hotelType });
                    newItems++;
                }
            }
...
for (const hotelType of hotelTypes) {
    const places = await collectPlacesForHotelType(hotelType);
